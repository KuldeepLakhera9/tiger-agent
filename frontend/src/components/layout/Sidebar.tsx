...
const links = [
  ...
  { label: 'Historical Cases', to: '/historical', icon: History },
...
export function Sidebar() {
  ...
  const isConnected = Boolean(health?.tigergraph?.connected);
  ...
  return (
    <aside className="hidden w-64 shrink-0 border-r border-slate-800 bg-slate-950/80 px-4 py-5 lg:flex lg:flex-col">
      <div className="flex items-center gap-3 px-3 pb-8">
        <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-violet-500/15 text-violet-300 ring-1 ring-violet-400/30">
          <TerminalSquare size={19} />
        </div>
        <div>
          <p className="text-sm font-semibold tracking-[0.22em] text-white">FRAUDGRAPH</p>
          <p className="mt-1 text-[10px] uppercase tracking-[0.18em] text-slate-500">Investigation OS</p>
        </div>
      </div>
      ...
      <div className="mt-auto rounded-xl border border-slate-800 bg-slate-900/70 p-4">
        <div className="mb-2 flex items-center justify-between">
          <span className="text-[10px] font-semibold uppercase tracking-[0.16em] text-slate-500">Graph Backend</span>
          <span
        ...
        <p className="text-xs font-semibold text-slate-200">
          {isConnected ? 'TigerGraph Cloud Active' : 'Deterministic Case Store'}
        </p>
        <p className="mt-1 text-[11px] leading-relaxed text-slate-400">
          {isConnected
            ? `Connected to graph ${health?.tigergraph?.graph || 'FraudInvestigation'}`
            : '20 benchmark cases verified with zero credentials required.'}
        </p>
      </div>
    </aside>
  );
